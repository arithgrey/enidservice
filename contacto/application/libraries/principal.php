<?php if (!defined('BASEPATH')) exit('No permitir el acceso directo al script');
	...
	class principal extends CI_Model implements  iRegistro{	 			
		...
		/**/
		function get_telefono_por_usuario($id_usuario){

			$query_get ="SELECT tel_contacto FROM usuario WHERE idusuario= $id_usuario LIMIT 1"; 
			$result =  $this->db->query($query_get);
			return $result->result_array()[0]["tel_contacto"];
		}
		/**/
		function get_departamentos($in_session){

			$query_get = "SELECT * FROM departamento";
			if ($in_session == 0){
				$query_get = "SELECT * FROM departamento WHERE visible_cliente_contacto = 1 ";		
			}
			$result =  $this->db->query($query_get);
