...
/**/
function get_btn_perfiles($id_usuario){
    if ($id_usuario == 1 ){
      return "<i data-toggle='modal' data-target='#fijar_metas_equipo_perfil'  class='fa fa-space-shuttle cargar_perfiles_disponibles' aria-hidden='true'></i>";

    }
  }
  /**/
  function get_btn_metas($id_usuario){
    if ($id_usuario == 1 ){
      return "<i data-toggle='modal' data-target='#fijar_metas_equipo'  class='fa fa-plus' aria-hidden='true'></i>";
    }
  }

if ( ! function_exists('valida_btn_productividad'))
{
  function valida_btn_productividad($id_usuario){
    
    $btn_productividad= "";
    if ($id_usuario ==  1){
      
      $btn_productividad= "<li> 
                        <a href='#tab_default_4' data-toggle='tab' class='productividad_usuario'> 
    ...
    return $btn_productividad;
  }
}
/**/
if ( ! function_exists('valida_btn_ingresos'))
{
  function valida_btn_ingresos($id_usuario){
    $btn_ingresos= "";
    if ($id_usuario ==  1){
      
      $btn_ingresos= "<li> 
            <a href='#tab_default_6' data-toggle='tab' class='reporte_ingresos'> 
...
function btn_busqueda_eventos(){
  ...
  $url = base_url("index.php/eventos/busqueda");
...
/**/
/**/
function create_select($data , $name , $class , $id , $text_option , $val , $seleccione_1 =0 ){

    $select ="<select name='". $name ."'  class='".$class ."'  id='". $id ."'> ";

      if ($seleccione_1 ==  1){
          $select .=  "<option value='-'>Selecciona una opción </option>";
      }
      foreach ($data as $row) {      
