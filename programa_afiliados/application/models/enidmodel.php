<?php defined('BASEPATH') OR exit('No direct script access allowed');
  class enidmodel extends CI_Model {
    ...
    function update_inicidencia($param){
      ...
      $query_update =  "UPDATE incidencia 
                        SET status = '".$status."' 
                        WHERE 
                        id_incidencia = '".$id_incidencia."' LIMIT 1 "; 
    ...
    function visitas_enid_semana($_num){
      ...
      $query_get  = " SELECT 
          ...
          $query_get = "SELECT 
          date(fecha_registro) f_registro , 
          count(0) total_registrado , 
          ...
          FROM  tmp_landing_$_num group  GROUP  BY f_registro                         
            UNION  
          ...
          SUM(CASE WHEN tipo = 45 then 1 else 0 end )pagina_web , 
          SUM(CASE WHEN tipo = 46 then 1 else 0 end )ultimos_proyectos                          
          FROM  tmp_landing_$_num group group  by totales";                 
