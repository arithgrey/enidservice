...
<div style="margin-top: 50px;">
	<table class='table_enid_service' border=1>		
		<tr  style="background: #000;color: white;text-align: center!important;">				
				...
				<?=get_th("Total")?>								
				<?=get_th("FAQ")?>
				<?=get_th("Formas de pago")?>			
				<?=get_th("Contacto")?>				
				<?=get_th("Sobre Enid")?>
				<?=get_th("Afiliados")?>
				<?=get_th("Home")?>								
				<?=get_th("Procesar compra" )?>												
		</tr>		
		<?=$info_uso;?>
		<tr style="background: #000;color: white;text-align: center!important;">				
				<?=get_th("Total")?>
				<?=get_th($total_visitas)?>								
				<?=get_th($total_faqs)?>
				<?=get_th($total_formas_pago)?>			
				<?=get_th($total_contacto)?>				
				<?=get_th($total_principal)?>
				<?=get_th($total_afiliado)?>
				<?=get_th($total_home)?>				
				<?=get_th($total_procesar_compra)?>		
