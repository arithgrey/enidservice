...
<div class="contenedor_global_servicio">
    ...
    <div class="tab-content">
        <?php $z[] = addNRow($notificacion_imagenes); ?>
        <?php $z[] = addNRow(valida_descartar_promocion($num_imagenes, $id_servicio, $id_perfil)) ?>
        <?php $z[] = div($images, ["class" => "contenedor_imagen_muestra"], 1) ?>
        <?php $z[] = heading_enid("¿TIENES ALGÚN VIDEO SOBRE TU " . $tipo_promocion . "?", 4) ?>
        <?php $z[] = div($text_llamada_accion_youtube, 1) ?>
        <?php $z[] = div($val_youtube, ["class" => "text_video_servicio"], 1) ?>
        <?php $z[] = get_form_youtube($valor_youtube) ?>
        <?= div(append($z), ["class" => "tab-pane " . $extra_1, "id" => "tab_imagenes"]) ?>

        <!--DESCRIPCION DEL PRODUCTO-->
        <?php $d[] = $nuevo_titulo_seleccion_producto; ?>
        <?php $d[] = place("place_tallas_disponibles"); ?>
        <?php $d[] = $info_nueva_descripcion; ?>
        <?php $d[] = get_form_descripcion($nueva_descripcion); ?>
        <?php $d[] = get_format_colores($flag_servicio, $info_colores); ?>
        <?= div(append($d), ["class" => "tab-pane " . $extra_2, "id" => "tab_info_producto"]) ?>
        <?php $t[] = div(div(estado_publicacion($status, $id_servicio), "col-lg-4 top_30 bottom_20", 1), 12) ?>
        <?php $t[] = get_form_rango_entrega($flag_servicio, $id_perfil, $stock) ?>
        <?php $t[] = div(get_rango_entrega(
                $id_perfil,
        ...
        <?php $t[] = get_seccion_uso_disponibilidad($existencia, $flag_nuevo, $flag_servicio) ?>
        <?php $t[] = get_seccion_ciclos_facturacion($ciclos, $id_ciclo_facturacion, $flag_servicio) ?>
        <?php $t[] = get_format_venta_mayoreo($flag_servicio, $text_envios_mayoreo, $venta_mayoreo) ?>
        <?php $t[] = get_format_enlace_venta_extra($flag_servicio, $url_ml) ?>
