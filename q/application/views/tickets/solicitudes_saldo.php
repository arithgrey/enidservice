<?= d(h("ULTIMOS MOVIMIENTOS", 1, ["class" => "titulo_enid"]), ["class" => "jumbotron"]) ?>
<?php if (count($solicitud_saldo) > 0): ?>
	<?= d("SOLICITUDES DE SALDO A TUS AMIGOS", 'titulo_enid_sm_sm' ) ?>
<?php endif; ?>
<?php foreach ($solicitud_saldo as $row): ?>
	<div class='list-group-item-movimiento '>
		<table>
			<tr>
				<?= get_td(d("Folio # " . $row["id_solicitud"], ["class" => 'folio']), ["colspan" => "2"]) ?>
			</tr>
			<tr>
				<td>
					<?=
