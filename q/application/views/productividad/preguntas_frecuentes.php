<?php
	...
	foreach ($lista_fechas as $row){		
		...
		if ($b == 0) {
			$fechas .=	get_th("Artículo" , "style='font-size:.8em;' ");
			$fechas .=	get_th("Total" , "style='font-size:.8em;' ");
			$b++;
		}

		$fecha_text  = $dias[date('N', strtotime($row) )]; 
		$fechas .= get_th( $fecha_text ."<br>".$row , "style='font-size:.8em;' ");
	}
	...
	foreach ($blogs as $row) {
		...
		$articulo ="<a href='".$url."' target='_black' style='font-size:.8em;'>".$titulo. "</a>";

		$list .="<tr class='text-left'>";	
			$list .=  "<td style='font-size:.8em;'>".$articulo."</td>";
				...
				foreach($lista_fechas as $row){		
					...
					$lista_2  .= get_td($num_accesos , "style='font-size:.8em;' class='text-center' ");
