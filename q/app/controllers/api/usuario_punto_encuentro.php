...
class Usuario_punto_encuentro extends REST_Controller
{
    ...
    function index_POST()
    {
        ...
        if (fx($param, "punto_encuentro,id_usuario")) {
            ...
            $num = $this->usuario_punto_encuentro_model->get_num($id_usuario,
                    $id_punto_encuentro);
    ...
    function quitar_PUT()
    {
        ...
        if (fx($param, "id_punto_encuentro,id_usuario")) {

            $in = [
                    "id_punto_encuentro" => $param['id_punto_encuentro'],
                    "id_usuario" => $param['id_usuario'],
            ];
            $set = ["status" => 0];
            $response = $this->usuario_punto_encuentro_model->update($set, $in, 10);
        }
        $this->response($response);
