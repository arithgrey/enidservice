...
class Home extends CI_Controller{
    ...
    function index(){
        ...
        $data = $this->val_session("¿Tienes problemas para contactar clientes?");
        if ($data["in_session"] ==  1 ) {            
            $num_perfil =  $this->sessionclass->getperfiles()[0]["idperfil"];                           
            if ($num_perfil == 20 ) {
                redirect("../../precios_sitios_web");
            }    
        }
        
        $data["meta_keywords"] = '';
        $data["desc_web"] = "¿Necesitas tener presencia en Internet?";                
        ...
        $resumen_respuestas = $this->principal->get_faqs_categoria(5, 0);
