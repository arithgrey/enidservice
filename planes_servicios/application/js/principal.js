...
function carga_servicios(){
	...
	data_send = {"q" : get_q() , "id_clasificacion" : get_id_clasificacion() , "page" : get_option("page")}
...
function carga_informacion_servicio(num){
		...
		$.ajax({		
			...
			$(".btn_agregar_termino").click(lista_terminos_disponibles);					
...
function agregar_termino_servicio(e){
	...
	data_send = $(".form_termino_servicio").serialize()+"&"+$.param({servicio : get_option("servicio")});  	
	url = "../base/index.php/api/servicio/terminos_servicios/format/json/";
...
function asocia_termino_servicio(e){
	...
	data_send = $.param({"servicio" : get_option("servicio") , "caracteristica" :  id_caracteristica});  		
...
function muestra_seccion_nombre_servicio(e){
	...
	if (visible ==  true){		
		showonehideone( ".input_nombre_servicio_facturacion",  ".text_nombre_servicio"  );	
	}else{
		showonehideone( ".input_nombre_servicio_facturacion",  ".text_nombre_servicio"  );	
	}		
}
...
function muestra_seccion_desc_servicio(e){		
	...
	if (visible ==  true){		
		showonehideone( ".input_desc_servicio_facturacion",  ".text_desc_servicio"  );	
	}else{
		showonehideone( ".input_desc_servicio_facturacion",  ".text_desc_servicio"  );	
	}		
}
...
function registra_costo_servicio(e){
		...
		data_send = $(".form_costo").serialize()+"&"+$.param({"servicio" : get_option("servicio")});				
		$.ajax({		
				url : url,
				type : "PUT", 		
				data : data_send,  			
...
function actualiza_dato_servicio(e){
		...
		data_send = $(".form_servicio_nombre_info").serialize()+"&"+$.param({"servicio" : get_option("servicio")});		
...
function actualiza_dato_url_venta(e){
		url = "../base/index.php/api/servicio/q/format/json/";
		data_send = $(".form_servicio_url_venta").serialize()+"&"+$.param({"servicio" : get_option("servicio")});		
...
function actualiza_dato_servicio_afiliado(e){
		...
		url = "../base/index.php/api/servicio/q/format/json/";
		data_send = $(".form_servicio_afiliados").serialize()+"&"+$.param({"servicio" : get_option("servicio")});		
...
function actualiza_dato_servicio_youtube(e){
		...
		if (valida_url_youtube()==1 ) {
			...
			url = "../base/index.php/api/servicio/q/format/json/";
			data_send = $(".form_servicio_youtube").serialize()+"&"+$.param({"servicio" : get_option("servicio")});				
			$.ajax({		
					url : url,
					type : "PUT", 		
					data : data_send,  			
...
function actualiza_dato_servicio_facebook(e){
	url = "../base/index.php/api/servicio/q/format/json/";
		data_send = $(".form_servicio_facebook").serialize()+"&"+$.param({"servicio" : get_option("servicio")});				
...
function actualiza_dato_servicio_desc(e){
		...
		url = "../base/index.php/api/servicio/q/format/json/";
		data_send = $(".form_servicio_desc").serialize()+"&"+$.param({"servicio" : get_option("servicio"),"q2" : $(".note-editable").html()});				
...
function registrar_ciclo_facturacion(e){
		...
		data_send = $(".form_ciclo_facturacion").serialize()+"&"+$.param({"servicio" : get_option("servicio")});		
...
function agrega_quita_servicio_grupo(e){
	...
	/**/
	data_send = $.param({"servicio" : get_option("servicio") , "grupo" : get_grupo()});  		
...
function muestra_seccion_porcentaje_ganancia(){
	...
	if (visible ==  true){		
		showonehideone( ".input_porcentaje_ganancia",  ".text_porcentaje_ganancia"  );	
	}else{
		showonehideone( ".input_porcentaje_ganancia",  ".text_porcentaje_ganancia"  );	
	}			
}
...
function muestra_seccion_porcentaje_ganancia_afiliados(){
	...
	if (visible ==  true){		
		showonehideone( ".input_porcentaje_ganancia_afiliados",  ".text_porcentaje_ganancias_afiliados"  );	
	}else{
		showonehideone( ".input_porcentaje_ganancia_afiliados",  ".text_porcentaje_ganancias_afiliados"  );	
	}				
}
...
function busqueda_terminos(e){
	...
	data_send=  $(".form_busqueda_termino").serialize()+"&"+$.param({"servicio" :get_option("servicio")});	
...
function actualiza_categorias_servicio(){
	...
	data_send = {"primer_nivel":get_categoria_primer_nivel() , "segundo_nivel":get_categoria_segundo_nivel() , "tercer_nivel":get_categoria_tercer_nivel() , "cuarto_nivel":get_categoria_cuarto_nivel() , "quinto_nivel":get_categoria_quinto_nivel(), "id_servicio" : get_option("servicio")};
...
function set_valores_categorias(){
	...
	if (get_option("nuevo")== 0 ){
		...
		/**/
		primer_nivel = $(".categorias_edicion .primer_nivel_seccion option:selected").val();
		set_categoria_primer_nivel(primer_nivel);
		/***/
		segundo_nivel = $(".categorias_edicion .segundo_nivel option:selected").val();
		if(segundo_nivel != undefined) {		
			set_categoria_segundo_nivel(segundo_nivel);
		}
		/***/
		tercer_nivel = $(".categorias_edicion .tercer_nivel option:selected").val();
		if(tercer_nivel != undefined) {
			set_categoria_tercer_nivel(tercer_nivel);	
		}
		cuarto_nivel = $(".categorias_edicion .cuarto_nivel option:selected").val();
		if(cuarto_nivel != undefined) {
			set_categoria_cuarto_nivel(cuarto_nivel);	
		}
		/**/
		quinto_nivel = $(".categorias_edicion .quinto_nivel option:selected").val();
...
function elimina_foto_producto(e){
	...
	data_send = {"id_imagen" : get_imagen()}
...
function actualiza_servicio_usado(){
	...
	url = "../base/index.php/api/servicio/q/format/json/";
	data_send = $.param({"servicio" : get_option("servicio") , "q2" : get_producto_nuevo() ,  "q" :"flag_nuevo" });				
...
function actualiza_envio_incluido(){
	...
	url = "../base/index.php/api/servicio/q/format/json/";
	data_send = $.param({"servicio" : get_option("servicio") , "q2" : get_envio_incluido() ,  "q" :"flag_envio_gratis" });				
	$.ajax({		
				url : url,
				type : "PUT", 		
				data : data_send,  			
...
function actualiza_ciclo_facturacion(){
	...
	url = "../base/index.php/api/servicio/ciclo_facturacion/format/json/";
	data_send = $.param({"servicio" : get_option("servicio") , "ciclo_facturacion" : get_option("id_ciclo_facturacion")});		
...
function actualiza_cantidad(){
	...
	url = "../base/index.php/api/servicio/q/format/json/";
	data_send = $.param({"servicio" : get_option("servicio") , "q2" : get_option("existencia"),  "q": "existencia"   });		
...
function agrega_color_servicio(e){
	...
	data_send = $.param({"servicio" : get_option("servicio") , "color": get_option("color")});
