<?php defined('BASEPATH') OR exit('No direct script access allowed');
  class productividad_usuario_model extends CI_Model {
    ...
    function get_adeudo_cliente($param){
        ...
        $result =  $this->db->query($query_get);        
        $id_persona =  $result->result_array()[0]["id_persona"];

        /**/
        $query_get="SELECT                         
                        SUM(ppf.monto_a_pagar)saldo_a_pagar,
                        SUM(ppf.saldo_cubierto)saldo_cubierto,                        
                        SUM(ppf.monto_a_pagar) - 
                        ...
                        WHERE 
                        pp.id_persona = '".$id_persona."' ";

        
        $result =  $this->db->query($query_get);        
        $saldo_pendiente =  $result->result_array()[0]["saldo_pendiente"];
