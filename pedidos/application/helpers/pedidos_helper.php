...
if (!function_exists('invierte_date_time')) {
    ...
    function get_format_resumen_cliente_compra($recibo, $tipos_entregas, $domicilio, $num_compras, $usuario, $id_recibo)
    {
        ...
        $r[] = br();
        $r[] = addNRow(div(create_seccion_saldos($recibo), ["class" => "padding_10 resumen_pago"]));
        $r[] = get_format_menu($domicilio, $recibo, $id_recibo);
        $r[] = br();
        $response = div(append_data($r), ["class" => "border padding_10"]);
        return $response;

    }

    if (!function_exists('get_forms')) {
        function get_forms($id_recibo, $lista_direcciones)
        {
            $r[] = div("DOMICILIOS DE ENTREGA ", ["class" => "text_direcciones_registradas"]);
            $r[] = agregar_nueva_direccion(0);
            $r[] = ul(create_lista_direcciones($lista_direcciones, $id_recibo), ["class" => "list-group list-group-flush"]);
            $r[] = br(2);
    ...
    if (!function_exists('agregar_nueva_direccion')) {
        function agregar_nueva_direccion($direccion = 1)
        {
            ...
            $response = li(guardar("Agregar direccion ",
                ["class" => "agregar_direccion_pedido"]));

            if ($direccion > 0) {

                $response = li(guardar("Agregar punto de encuentro ",
    ...
    if (!function_exists('get_lista_puntos_encuentro')) {
        function get_lista_puntos_encuentro($puntos_encuentro, $id_recibo, $domicilio = '')
        {
            ...
            foreach ($puntos_encuentro as $row) {
                ...
                $lista[] = li(append_data($encuentro), ["class" => "list-group-item " . $extra]);

                $a++;
            }

            return append_data($lista);
    ...
    if (!function_exists('valida_accion_pago')) {
        function valida_accion_pago($recibo)
        {
            ...
            if ($recibo["saldo_cubierto"] < 1) {
                ...
                $response = guardar("PROCEDER A LA COMPRA " . icon("fa fa-2x fa-shopping-cart"),
                    [
    ...
    if (!function_exists('create_seccion_comentarios')) {
        function create_seccion_comentarios($data, $id_recibo)
        {
            ...
            $nota[] = heading_enid("NOTAS", 4, ["class" => "white"]);
            return div(append_data($nota), ["class" => "global_tipificaciones"]);
    ...
    if (!function_exists('create_seccion_tipo_entrega')) {
        function create_seccion_tipo_entrega($recibo, $tipos_entregas)
        {
            ...
            $tipo = div($tipo, ["class" => "encabezado_tipo_entrega"], 1);
    ...
    if (!function_exists('create_seccion_usuario')) {
        function create_seccion_usuario($usuario)
        {
            ...
            $text_usuario = div(append_data($text), ["class" => "contenido_domicilio"]);
    ...
    if (!function_exists('resumen_compras_cliente')) {
        function resumen_compras_cliente($num)
        {
            ...
            return div($text . $starts, ["class" => "compras_en_tiempo padding_20"]);
